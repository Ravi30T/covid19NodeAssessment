...
const checkRequestQueries = async (request, response, next) => {
  ...
  if (date !== undefined) {
    try {
      ...
      const formattedDate = dateFormat(new Date(myDate), 'yyyy-MM-dd')
      ...
      const isValidDate = await dateValid(result)
...
const checkRequestsBody = (request, response, next) => {
  ...
  if (dueDate !== undefined) {
    try {
      ...
      const isValidDate = dateValid(result)
...
app.put('/todos/:todoId', checkRequestsBody, async (request, response) => {
  ...
  switch (true) {
    ...
    case category !== undefined:
      const categoryItem = `UPDATE todo SET category = '${category}' WHERE id = ${todoId};`

      await db.run(categoryItem)
    ...
    case dueDate !== undefined:
      const dateItem = `UPDATE todo SET due_date = '${dueDate}' WHERE id = ${todoId};`

      await db.run(dateItem)
